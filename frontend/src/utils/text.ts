...
/**
 * Détecte si l'option contient au moins une série de type pie/donut.
 */
function hasPieSeries(option: EChartsOption): boolean {
  const series = option.series;
  if (!series) return false;
  const arr = Array.isArray(series) ? series : [series];
  return arr.some((s) => (s as Record<string, unknown>)?.type === 'pie');
}

/**
 * Fusionne les defaults ECharts avec l'option générée par le LLM.
 * Pour les pie/donut charts, la légende est placée à gauche en vertical
 * et le centre du camembert est décalé pour éviter tout chevauchement.
 */
export function mergeEChartsOption(option: EChartsOption): EChartsOption {
  ...
  if (hasPieSeries(option)) {
    // Légende verticale à droite pour les camemberts
    merged.legend = {
      ...baseLegend,
      ...userLegend,
      orient: 'vertical',
    ...
    // Décaler le centre des séries pie vers la gauche pour laisser de la place à la légende
    const series = Array.isArray(option.series) ? option.series : option.series ? [option.series] : [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    merged.series = (series.map((s) => {
      const sr = s as Record<string, unknown>;
      if (sr?.type !== 'pie') return s;
      // Ne pas écraser un center explicitement défini par le LLM
      const hasExplicitCenter = Array.isArray(sr.center) && (sr.center as unknown[]).length === 2;
      return {
        ...s,
        center: (hasExplicitCenter ? sr.center : ['40%', '50%']) as [string, string],
        radius: (sr.radius ?? ['0%', '65%']) as string | string[],
      };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    }) as any);

    // Retirer le grid qui ne s'applique pas aux pie charts
