...
export type ArtifactType = 'full' | 'table' | 'word' | 'chart' | 'mermaid' | 'excalidraw' | 'svg';
...
export function reconstructWordContent(
  ...
  while (pos < after.length) {
    ...
    if (lang === 'echarts' || lang === 'mermaid' || lang === 'excalidraw') {
...
export function extractArtifacts(content: string, msgIndex: number): Artifact[] {
  ...
  // Excalidraw (legacy) + SVG
  const excalidrawRegex = /```(?:excalidraw|svg)\n([\s\S]*?)```/g;
  let xMatch: RegExpExecArray | null;
  let xIdx = 0;
  while ((xMatch = excalidrawRegex.exec(content)) !== null) {
    xIdx++;
    // Déterminer le type réel selon la balise
    const tag = xMatch[0].startsWith('```svg') ? 'svg' : 'excalidraw';
    artifacts.push({ id: `excalidraw-${msgIndex}-${xIdx}`, type: tag, label: `Schéma ${xIdx}`, icon: '✏️', content: xMatch[1].trim() });
  }
