...
// ── ErrorBoundary générique pour les blocs visuels ───────────────────────────
interface EBState { hasError: boolean; message: string }
class VisualErrorBoundary extends React.Component<{ label: string; children: React.ReactNode }, EBState> {
  ...
  render() {
    if (this.state.hasError)
      return <div className="echarts-error"><span>⚠ {this.props.label}</span><pre>{this.state.message}</pre></div>;
    return this.props.children;
  }
}

// (Excalidraw supprimé — remplacé par renderer SVG natif, voir SvgBlock ci-dessous)
...
function getMermaidSandbox(): HTMLElement {
  ...
  if (!el) {
    ...
    el.style.cssText = 'position:fixed;top:-9999px;left:-9999px;width:1200px;height:auto;overflow:visible;visibility:hidden;';
...
// ── Mermaid  ─────────────────────────────────────────────────────────────
interface MermaidBlockProps { code: string; streaming?: boolean; svgRef?: React.MutableRefObject<string | null>; }

export const MermaidBlock = memo(function MermaidBlock({ code, streaming, svgRef }: MermaidBlockProps) {
  ...
  useEffect(() => {
    ...
    mermaid.render(id, sanitized, sandbox)
      .then(({ svg: rendered }) => {
        if (activeRender.current !== thisRender) return;
        // Supprimer width/height fixes générés par Mermaid pour que le SVG
        // soit responsive via CSS. Conserver uniquement le viewBox.
        const responsive = rendered
          .replace(/(<svg[^>]*?)\s+width="[^"]*"/,  '$1')
          .replace(/(<svg[^>]*?)\s+height="[^"]*"/, '$1')
          .replace(/(<svg[^>]*?)\s+style="[^"]*max-width:[^"]*"/, '$1');
        setSvg(responsive);
        if (svgRef) svgRef.current = responsive;
      })
      .catch((err: unknown) => {
...
// ── SvgBlock — renderer SVG inline ────────────────────────────────────────────
// Le LLM génère du SVG brut dans un bloc ```svg.
// Affiche le SVG avec zoom/pan natif et contrôles de copie/téléchargement.
interface SvgBlockProps { code: string; streaming?: boolean; }

/** Sanitise le SVG et force quelques attributs de sécurité/qualité */
function sanitizeSvg(raw: string): string {
  ...
  // Supprimer width/height absolus — le CSS prend le relais via width:100%/height:auto
  s = s.replace(/(<svg\b[^>]*)\s+width="[0-9.]+(?:px|pt|cm|in|em|rem)?"/i, '$1');
  s = s.replace(/(<svg\b[^>]*)\s+height="[0-9.]+(?:px|pt|cm|in|em|rem)?"/i, '$1');
  // Supprimer aussi le style inline max-width que Mermaid/autres injectent parfois
  s = s.replace(/(<svg\b[^>]*)\s+style="[^"]*max-width:[^"]*"/i, '$1');

  // Garantir preserveAspectRatio pour un scaling propre
  if (!/preserveAspectRatio/.test(s.slice(0, 400))) {
    s = s.replace(/(<svg\b[^>]*)(>)/i, '$1 preserveAspectRatio="xMidYMid meet"$2');
  }

  // Garantir font-family dans le style inline
  if (!/font-family/.test(s.slice(0, 400))) {
    s = s.replace(
      /(<svg\b[^>]*)(>)/i,
      '$1 style="font-family:Inter,\'Segoe UI\',Arial,sans-serif"$2'
    );
  }
...
export const SvgBlock = memo(function SvgBlock({ code, streaming }: SvgBlockProps) {
  const [svg, setSvg]         = useState<string | null>(null);
  const [error, setError]     = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!trimmed.toLowerCase().startsWith('<svg')) {
        throw new Error('Le contenu ne commence pas par <svg>');
      }
      setSvg(sanitizeSvg(trimmed));
    } catch (e) {
      console.error('[SVG] erreur :', (e as Error).message);
  ...
  /** Télécharge le SVG en fichier .svg */
  ...
  if (streaming) return (
    <div className="echarts-wrapper echarts-loading">
      <span className="echarts-spinner" /><span>Génération du schéma…</span>
    </div>
  );
  if (error) return (
    <div className="echarts-error">
      <span>⚠ Erreur SVG — {error}</span>
      <details style={{ marginTop: 6 }}>
        <summary style={{ cursor: 'pointer', fontSize: 12 }}>Source (debug)</summary>
        <pre style={{ fontSize: 11, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>{code}</pre>
      </details>
    </div>
  );
  if (!svg) return (
    <div className="echarts-wrapper echarts-loading"><span className="echarts-spinner" /></div>
  );
  return (
    <VisualErrorBoundary label="Erreur SVG">
      <div className="svg-wrapper" ref={wrapRef}>
        <div className="svg-toolbar">
          <button className="svg-toolbar-btn" onClick={handleCopySvg} title="Copier le SVG">
            {copied ? '✓ Copié' : '⎘ Copier SVG'}
          </button>
          <button className="svg-toolbar-btn" onClick={handleDownloadSvg} title="Télécharger en .svg">
            ↓ .svg
          </button>
        </div>
        <div className="svg-canvas" dangerouslySetInnerHTML={{ __html: svg }} />
      </div>
    </VisualErrorBoundary>
  );
});

// Alias — ArtifactsPanel importe ExcalidrawBlock, on le redirige vers SvgBlock
export const ExcalidrawBlock = SvgBlock;
...
export function CodeBlock({ className, children, node, streaming }: CodeBlockProps) {
  ...
  if (lang === 'svg')         return <SvgBlock code={code} streaming={streaming} />;
  if (lang === 'excalidraw')  return <SvgBlock code={code} streaming={streaming} />; // legacy alias
