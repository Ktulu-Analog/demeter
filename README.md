...
| 📚 **RAG** | Ingestion PDF/DOCX, recherche sémantique + reranking via Albert ; collections mises en cache (TTL 1 h) |
...
La liste des collections est **mise en cache localement** (TTL 1 heure) pour un affichage instantané. Survolez une collection pour afficher ses métadonnées (propriétaire, nombre de documents, dates). Le bouton ↺ force le rechargement depuis l'API avant expiration du cache.
